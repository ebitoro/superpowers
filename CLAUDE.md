...
### Files
- `.codex-state/codex_thread_id` - Persistent Codex thread ID for design/plan phases (managed by codex-agent)
- `.codex-state/codex_review_thread_id` - Ephemeral Codex thread ID for implementation reviews. Fresh per review gate, reused across retries (max 5), deleted after gate resolves. Survives compaction.
